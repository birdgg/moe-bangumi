...
function GroupReorderItem({ group, index, onRemove, onAddAlias, onRemoveAlias }: GroupReorderItemProps) {
  ...
  return (
    <Reorder.Item
      ...
      className="group/item flex items-center gap-2 rounded-lg border border-border/50 bg-muted/30 px-2 py-1.5 data-dragging:z-50 data-dragging:border-chart-1/50 data-dragging:bg-muted/80 data-dragging:shadow-lg"
    >
      <span
        className={cn(
      ...
      <div className="flex flex-1 items-center gap-1 overflow-hidden">
        {group.aliases.map((alias, aliasIdx) => (
          <span
            key={alias}
            className="inline-flex shrink-0 items-center gap-0.5 rounded-md bg-muted px-1.5 py-0.5 text-xs text-muted-foreground"
          >
            {alias}
            <button
              type="button"
        ...
        <Input
          ...
          className="h-5 min-w-16 flex-1 border-none bg-transparent px-1 text-xs shadow-none focus-visible:ring-0"
        />
      </div>
