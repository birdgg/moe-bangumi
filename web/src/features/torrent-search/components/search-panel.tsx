...
import type { TorrentSearchResult } from "@/client/types.gen";
...
function ResultItem({
  ...
  onDownload: (url: string, infoHash?: string) => void;
}) {
  ...
  return (
    <Command.Item
      ...
      onSelect={() => {
        if (item.torrentUrl)
          onDownload(item.torrentUrl, item.infoHash ?? undefined);
      }}
...
interface SearchPanelProps {
  ...
  results: TorrentSearchResult[] | undefined;
  isLoading: boolean;
  isFetching: boolean;
  debouncedKeyword: string;
  onSelectResult: (torrentUrl: string, infoHash?: string) => void;
}
...
export function SearchPanel({
  ...
  return (
    <Command shouldFilter={false} loop className="flex flex-col h-full">
      ...
      <div className="absolute inset-x-0 top-0 h-24 rounded-tl-2xl pointer-events-none bg-[radial-gradient(ellipse_80%_50%_at_50%_-20%,oklch(1_0_0_/_0.18),transparent_70%)] dark:bg-[radial-gradient(ellipse_80%_50%_at_50%_-20%,oklch(1_0_0_/_0.05),transparent_70%)]" />
      ...
      {/* Active filter tags */}
      <div className="relative flex items-center gap-1.5 px-4 pb-2.5">
        {["BDRip", "VCB-Studio"].map((tag) => (
          <span
            key={tag}
            className="glass-pill rounded-md px-2 py-0.5 text-[11px] font-medium bg-chart-2/12 text-chart-2"
          >
            {tag}
          </span>
        ))}
      ...
      {/* Results */}
      <Command.List className="no-scrollbar flex-1 overflow-y-auto px-1.5 py-1.5 scroll-smooth">
        {showSpinner && (
