...
runNotificationDynamic ::
  ...
  Eff es a
runNotificationDynamic = interpret $ \_ -> \case
    ...
    if cfg.botToken == ""
      then pass
        ...
        result <- case mImageUrl of
          Nothing -> Client.sendTelegramMessage mgr cfg.botToken cfg.chatId message
          Just imageUrl -> Client.sendTelegramPhoto mgr cfg.botToken cfg.chatId message imageUrl
        case result of
