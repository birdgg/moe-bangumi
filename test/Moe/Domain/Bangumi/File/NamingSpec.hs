...
namingTests =
  [ testCase "regular episode S01E01" $ do
    ...
    testCase "episode with group" $ do
      let meta = BangumiMeta "Frieren" (Just 2023) Nothing
          file = BangumiFile meta (Episode (Regular 1 1)) (Video MKV) [GroupName "SweetSub"]
      generateFullPath file @?= "Frieren (2023)/Season 01/Frieren - S01E01 [SweetSub].mkv",
    testCase "episode with multiple groups" $ do
      let meta = BangumiMeta "Frieren" (Just 2023) Nothing
          file = BangumiFile meta (Episode (Regular 1 1)) (Video MKV) [GroupName "喵萌奶茶屋", GroupName "LoliHouse"]
